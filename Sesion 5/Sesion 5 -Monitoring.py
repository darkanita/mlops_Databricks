...
train_df['target'] = housing.target
...
train_df['weather_condition'] = np.random.choice(['sunny', 'cloudy', 'rainy'], size=size, p=[0.5, 0.3, 0.2])
...
df_numerical_features = train_df.select_dtypes(include='number').columns
df_categorical_features = train_df.select_dtypes(include='object').columns
...
pdf1 = train_df.sample(frac = 0.5, random_state=1)
pdf2 = train_df.drop(pdf1.index)

# COMMAND ----------

pdf2["target"] = 2 * pdf2["target"]
pdf2["HouseAge"] = pdf2["HouseAge"] / 20
pdf2["MedInc"] = pdf2["MedInc"].map(lambda x: None if x < 4 else x)
...
summary1_pdf = pdf1.describe()
summary2_pdf = pdf2.describe()
...
alpha = 0.05
alpha_corrected = alpha / len(df_numerical_features)

# Loop over all numeric attributes (numeric cols and target col, price)
for num in df_numerical_features:
...
alpha = 0.05
corrected_alpha = alpha / len(df_categorical_features) # Still using the same correction
...
drift_monitor = Monitor(pdf1, pdf2, df_categorical_features, df_numerical_features)
drift_monitor.run()
